...
// This is foolish but is unlikely to cause major issues  
int line;
...
void translate_V_V_instruction(char* instruction, char* opcode, char leader, char finisher, unsigned short instruction_len) {
        ...
        instruction = skip_spaces(instruction + 2) + 1;
...
void translate_V_NN_instruction(char* instruction, char* opcode, char leader, unsigned short instruction_len) {
        ...
        instruction = skip_spaces(instruction + instruction_len) + 1;
        X = (unsigned char) strtol(instruction, NULL, 16);
        instruction = skip_spaces(instruction + 2);
...
void translate_Z_V_instruction(char* instruction, char* opcode, char leader, char finisher, unsigned short instruction_len, unsigned short Z_len) {
        unsigned char X;
        instruction = skip_spaces(skip_spaces(instruction + instruction_len) + Z_len + 1) + 1;
        X = (unsigned char) strtol(instruction, NULL, 16);
        opcode[0] = (leader << 4) | X;
        opcode[1] = finisher;
}
...
int translate_instruction(char* instruction, char* opcode){
    ...
    if(!regexec(&CLS_re, instruction, 0, NULL, 0)) {
        ...
        translate_NNN_instruction(instruction, opcode, 0x2, 4);
    }
        ...
        translate_V_V_instruction(instruction, opcode, 0x5, 0x0, 2);
    }
        ...
        translate_V_instruction(instruction, opcode, 0xF, 0x07, 2);
    }
    else if(!regexec(&LD_V_K_re, instruction, 0, NULL, 0)) {
        translate_V_instruction(instruction, opcode, 0xF, 0x0A, 2);
    }
        ...
        translate_Z_V_instruction(instruction, opcode, 0xF, 0x55, 2, 3);
    }
    else if(!regexec(&LD_V_I_re, instruction, 0, NULL, 0)) {
        translate_V_instruction(instruction, opcode, 0xF, 0x65, 2);
    }
    else {
        return -1;
    }
...
int main(int argc, char** argv) {
    ...
    //compile regular expression so that they will be available for use
